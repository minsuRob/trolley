...
**본문은 인덱싱 단계에서 아예 읽지 않는다.** 이게 이 기능의 전부다. 위키는 `.md` 160개에
411KB이고 통째로는 10만 토큰이 넘는데, 서버의 실측 소프트 한계(`context_guard.py`의
`SOFT_CONTEXT_LIMIT`)가 96,000이다. 그래서 파일마다 **앞 8KB만 읽고 프론트매터 블록에서
멈춘다** — 실측상 가장 긴 프론트매터도 400바이트 언저리다. 첫 줄이 `---`가 아닌 파일
(`INDEX.md` 21KB, `CLAUDE.md` 20KB, `logs/**` 전부 — 위키 바이트의 절반 이상)은 그 검사
하나로 걸러진다. 실제 볼트 110개 페이지를 걷는 데 **25ms**가 든다.
...
| --- | --- | --- | --- | --- |
| `상태=진행중` (기본값) | 65/65 | 7,588 | ~3.4K | **3.6%** |
| 전체 | 73/110 (예산에서 잘림) | 8,871 | ~4.0K | 4.2% |
| 전체, 요약 제외 | 110/110 | 7,970 | ~3.6K | 3.8% |
| 진행중 + 담당 1명 | 14/14 | 2,171 | ~1.0K | 1.0% |
| 진행중 + 최우선 | 3/3 | 460 | ~0.2K | 0.2% |

기본 문자 예산은 9,000자다. 처음엔 4,000으로 뒀다가 진행중 65건 중 35건이 조용히 잘리는
것을 보고 올렸다 — **줄어든 목록은 모델에게 완전한 목록으로 읽히고, 보여준 적 없는 일감에
대해 "그게 전부"라고 답하게 된다.** 예산을 넘기면 줄 단위로 자르고 몇 건이 빠졌는지 본문에
적는다. `요약 포함`을 끄면 볼트 전체가 같은 예산에 들어간다 — 문자 수를 가장 크게 좌우하는
스위치다.
...
패널의 힌트 줄이 지금 상태를 그대로 말해준다(`위키 65건 첨부` / `이미 전달됨` / `새 대화부터 반영`).

**필터**는 위키의 한글 프론트매터를 그대로 쓴다 — `유형`·`상태`·`분류`·`우선순위`는 닫힌
목록이라 팝업이고, `영역`·`담당`은 열린 값이라 자유 입력이다(새 멤버가 합류한 날 필터에서
사라지지 않게). 옵션 창 아래쪽 **미리보기가 이 기능의 요점**이다: 컨트롤을 움직일 때마다
`65/65건 · 7,588자 · 예산의 84% · 96K 컨텍스트의 3.6%`가 다시 계산된다. 비용을 보여주지
...
`trolley wiki --json`의 106행이 `INDEX.md`의 106행과 **순서까지 일치**한다.
...
한 줄 목록)와 `wiki_read`(제목 하나로 본문). **본문을 읽는 곳은 `wiki_read` 뿐이다.**
제목은 경로가 아니라 인덱스에서 basename으로 찾으므로, 인덱스 밖으로 나가는 경로를 막는 게
아니라 애초에 그런 페이지가 존재하지 않는다. 위키를 끄거나 폴더가 없으면 두 툴은 목록에
