...
창은 직접 닫는다 — 스스로 닫히지 않는다. 닫아도 앱은 살아 있고 펫이 남는다. 다시 열려면
**펫을 우클릭 → 설정 열기**, 또는 Finder 에서 앱을 다시 열면 된다(Dock 아이콘이 없다).
터미널에서는 `trolley setup`. 준비가 다 끝난 뒤로는 앱을 켜도 창이 따라 열리지 않는다.
