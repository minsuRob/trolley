...
**스테이플은 재시도한다.** 공증이 `Accepted`로 끝난 직후에도 한동안
`Could not validate ticket`으로 실패한다 — 티켓은 발급돼 있지만(`notarytool log`의
`ticketContents`에 dmg의 cdhash가 들어 있다) Apple의 배포 엔드포인트가 아직 서빙하지
않는 구간이 있다. 1분 간격으로 열 번까지 다시 시도하고, 끝내 안 되면 경고만 남긴다 —
인터넷이 있는 맥에서는 온라인 확인으로 통과하므로 배포 자체는 가능하다.
...
- **우클릭**: PID 표시 + **위젯 종료** — 위젯과 MCP 서버가 한 프로세스이므로
  `trolley mcp` 자체가 종료된다. 숨기기는 없다(숨기면 되살릴 수단이 없어 보이지
  않는 프로세스만 남으므로).
