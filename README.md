...
#### LLM 위키: 팀 위키를 질문 앞에 얹기

`markhub-llm-wiki`(팀의 마크다운 위키, Obsidian 볼트) 폴더를 지정해두면 **로컬 LLM에게
묻는 질문 앞에 위키 요약이 자동으로 붙는다.** 설정 창의 "LLM 위키" 행 → **설정**에서
폴더를 고르고 필터를 정한다. 기본값은 **꺼짐**이다.
...
**대화당 한 번만 붙는다.** 서버는 대화를 계속 쌓아두고 매 턴 전체를 다시 prefill하므로,
...
**에이전트도 쓸 수 있다.** 위키를 켜두면 MCP 툴 두 개가 함께 열린다 — `wiki_search`(필터로
한 줄 목록, 같은 `detail` 축을 인자로 받는다)와 `wiki_read`(제목 하나로 본문). **본문을 읽는
